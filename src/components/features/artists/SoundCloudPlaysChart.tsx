...
import type { VibrateSoundCloudPlaysData } from '../../../services/vibrateService';
import { processTimeSeriesData, formatNumber, calculateGrowth } from './chartUtils';

interface SoundCloudPlaysChartProps {
  data: VibrateSoundCloudPlaysData;
  loading?: boolean;
}

const SoundCloudPlaysChart = ({ data, loading = false }: SoundCloudPlaysChartProps) => {
  const chartData = processTimeSeriesData(data.plays);
                  ...
                  return (
                    <div className="bg-white p-2 rounded shadow-lg border text-xs">
                      ...
                      <div className="text-orange-600">
                        Plays: {formatNumber(payload[0].value as number)}
                      </div>
                    </div>
                  );
                }
...
export default SoundCloudPlaysChart;
