...
const PercentSoldDropdown: React.FC<PercentSoldDropdownProps> = ({ value, onChange }) => {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        type="button"
        className="w-full flex items-center justify-between px-3 py-2 border border-gray-300 rounded-lg bg-white text-left focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500"
        onClick={() => setOpen((prev) => !prev)}
      >
        <span className="text-gray-900">{formatPercentRange(value)}</span>
        <svg
