...
const ArtistDetails = () => {
  ...
  // UI state for audience countries - MUST be before any conditional returns
  ...
  console.log(`🎨 ArtistDetails: Component render #${renderCount.current} started`);
  
  // Use RTK Query for all data fetching with comprehensive caching
  const { 
    data: artistData,
    isLoading,
    isError,
  ...
  // Loading states
  if (!user) {
    console.log('❌ ArtistDetails: No user, redirecting');
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-accent-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Please log in to view artist details</p>
        </div>
      </div>
    );
  }
  ...
  if (isError || !artistData) {
    console.log('❌ ArtistDetails: Error state or no data');
    return (
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-x-hidden">
          <div className="rounded-3xl bg-white shadow-soft p-4 lg:p-8 min-h-[90vh] max-w-full">
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 mb-6">The artist you're looking for doesn't exist or couldn't be loaded.</p>
              <button 
                onClick={() => navigate('/artist-search')}
  ...
  const getCityYAxisWidth = () => {
    if (!spotifyListeners?.byCity?.length) return 80;
        ...
        <div className="rounded-3xl bg-white shadow-soft p-4 lg:p-8 min-h-[90vh] max-w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="bg-white rounded-2xl shadow p-8 flex flex-col items-center gap-6 border border-gray-100">
                ...
                {vibrateArtist?.image && (
                  <img
                    ...
                    alt={artist?.name || vibrateArtist?.name || 'Artist'}
                ...
                <div className="w-full text-center">
                  <h1 className="text-2xl font-bold text-gray-900 mb-1">{artist?.name || vibrateArtist?.name || 'Unknown Artist'}</h1>
                  ...
                  {!vibrateArtist?.verified && vibrateArtist && (
                    <div className="flex items-center justify-center gap-1 mb-2">
                      <span className="text-xs text-green-600 font-medium">✓ Found in Vibrate Database</span>
                    </div>
                  )}
                ...
                {vibrateBio && vibrateBio.BIO.length > 0 && (
                  <div className="w-full mt-6">
            ...
            <div className="lg:col-span-9 space-y-8">
              ...
              <div id="analytics-city-section" className="mt-8">
                {/* Spotify Listeners by City */}
                {spotifyListeners?.byCity && spotifyListeners.byCity.length > 0 && (
                  <div className="w-full">
                ...
                {instagramAudience?.byCity && instagramAudience.byCity.length > 0 && (
                  <div className="w-full">
              ...
              <div id="analytics-gender-section" className="mt-8">
                {/* Instagram Gender & Age Analytics */}
                {Object.keys(instagramAudience?.byGender || {}).length > 0 && Object.keys(instagramAudience?.byAge || {}).length > 0 ? (
                  ...
                  <>
                    {/* Fallback: render separately if only one is present */}
